//! Running the CUDA backend with no CUDA, to check what it COMPUTES.
//!
//! `emulate_bang.rs` did this for Cambricon and found all four of its row operations
//! reducing the whole tile instead of each row. The same lever exists for `gpu`: its
//! emitted CUDA is ordinary C once `__global__`, `threadIdx` and the math intrinsics are
//! defined, so a stub that EMULATES rather than no-ops turns the kernel into a function
//! this machine can run.
//!
//! It is driven with the same generator `run::input_values_for` uses and compared against
//! `run::reference_output` — the same reference the Metal and Vulkan paths use, so the
//! four cannot drift apart.
//!
//! WHAT IT COVERS, AND WHY NOT MORE. Only ELEMENTWISE f32 ops. Two hard limits, both
//! properties of a single-threaded emulator rather than choices:
//!
//!   * the reductions (`softmax`, `reduce_sum`, `reduce_max`, `absmax`) lower to warp
//!     primitives — `__shfl_down_sync` across 32 lanes. Running one thread at a time
//!     cannot produce a warp, and a stub that returns the value unchanged would compute a
//!     confident wrong answer rather than fail. They are skipped, not faked.
//!   * `__half` is typed as `float` in the stub, because the emitted prologue casts between
//!     them and a struct cannot be cast. So f16 kernels would run at f32 precision, and
//!     only f32 kernels are driven.
//!
...
/// The CUDA runtime this machine does not have, EMULATED rather than no-oped.
///
/// It lives in `testdata/emu/` rather than in this file because `emulate_musa.rs` needs the
/// identical stub under a different name: Moore Threads' MUSA emits the same `__global__`,
/// `threadIdx.x` and `expf` vocabulary and differs from CUDA by seven to nine lines per
/// kernel, so it is a genuinely different lowering answered by the same shim. Two copies of
/// an emulator would drift, and a drifted emulator reports a defect in whichever backend
/// was checked against the older one.
const CUDA_EMU: &str = include_str!("../testdata/emu/cuda_runtime.h");
...
#[test]
fn gpu_computes_what_the_reference_computes() {
    let Some(cc) = which("clang").or_else(|| which("gcc")) else {
        eprintln!("emulate_gpu: skipped, no host C compiler");
        return;
    };
    let exe = env!("CARGO_BIN_EXE_tile");
    let dir = std::env::temp_dir().join(format!("tile-emulate-gpu-{}", std::process::id()));
    std::fs::create_dir_all(&dir).expect("a scratch directory");
    // The emitted CUDA does `#include <cuda_runtime.h>`, so the stub must answer to THAT
    // name on the include path. `cuda_fp16.h` is a second include the f16 preamble adds;
    // pointing it at the same shim avoids redefining `__half`.
    std::fs::write(dir.join("cuda_runtime.h"), CUDA_EMU).expect("write stub");
    std::fs::write(
    ...
    eprintln!("emulate_gpu: {checked} kernels run and compared, {refused} refused");
    ...
    assert!(
        checked >= 20,
        "only {checked} kernels were emitted and run -- gpu is refusing nearly everything \
         and this test would pass by checking nothing"
    );
    assert!(
        bad.is_empty(),
        "{} emulated CUDA kernels disagree with the reference. This is not a syntax \
