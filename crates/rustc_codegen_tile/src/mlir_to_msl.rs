...
pub fn convert_mlir_to_msl(mlir_text: &str) -> Result<String, String> {
    ...
    let needs_e4m3fn = module.functions.iter().any(|f| {
        f.body_lines.iter().any(|l| {
            l.contains("__tile_kv_fp8_store_f32")
                || l.contains("__tile_fp8_kv_quantize_f32")
        })
    });
    if needs_e4m3fn {
    ...
    let needs_gelu_consts = module.functions.iter().any(|f| {
        f.body_lines.iter().any(|l| l.contains("__tile_gelu_f32_4") || l.contains("__tile_gelu_f16") || l.contains("__tile_gelu_f32_scalar"))
    });
    ...
    for line in body_lines {
        ...
        if line.contains("llvm.call @") {
            ...
            match callee.as_str() {
                ...
                "__tile_softmax_f32"    | "__tile_softmax_f16"    => { ctx.kernel_type = KernelType::Softmax; }
                ...
                "__tile_sub_f32"        | "__tile_sub_f16"        => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::Sub; } }
                ...
                "__tile_exp_f32"                                          => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::Exp; } }
                "__tile_scale_f32"      | "__tile_scale_f16"      => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::Scale; } }
                "__tile_layernorm_f32"  | "__tile_layernorm_f16"  => { ctx.kernel_type = KernelType::LayerNorm; }
                "__tile_l2dist_f32"                                    => { ctx.kernel_type = KernelType::L2Dist; }
                "__tile_argmin_f32"                                    => { ctx.kernel_type = KernelType::Argmin; }
                ...
                "__tile_sqrt_f32"                                         => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::Sqrt; } }
                "__tile_softplus_f32"                                     => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::Softplus; } }
                "__tile_clamp_f32"                                        => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::Clamp; } }
                "__tile_cast_f32_f16"                                     => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::CastF32F16; } }
                "__tile_cast_f16_f32"                                     => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::CastF16F32; } }
                "__tile_slice_f32"                                        => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::Slice; } }
                "__tile_concat_f32"                                       => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::Concat; } }
                "__tile_scatter_f32"                                      => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::Scatter; } }
                ...
                "__tile_rms_norm_f32"   | "__tile_rms_norm_f16"   => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::RmsNorm; } }
                "__tile_absmax_f32"     | "__tile_absmax_f16"     => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::Absmax; } }
                "__tile_quantize_f32_i8"                               => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::Quantize; } }
                "__tile_dequantize_i8_f32"                             => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::Dequantize; } }
                // Phase 6 MTP ops
                "__tile_argmax_f32"        => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::ArgMax; } }
                "__tile_sample_top_p_f32"  => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::SampleTopP; } }
                "__tile_draft_verify_f32"  => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::DraftVerify; } }
                ...
                "__tile_topk_mask_scatter_f32" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::TopkMaskScatter; } }
                "__tile_router_weights_one_f32" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::Dsv4RouterWeightsOne; } }
                "__tile_indexer_weighted_sum_f32" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::Dsv4IndexerWeightedSum; } }
                "__tile_sort_i32_rows_asc_i32" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::SortI32RowsAsc; } }
                "__tile_softmax_pool_f32" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::Dsv4SoftmaxPool; } }
                "__tile_compressor_store_one_f32" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::Dsv4CompressorStoreOne; } }
                "__tile_kv_fp8_store_f32" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::Dsv4KvFp8Store; } }
                "__tile_fp8_kv_quantize_f32" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::Dsv4Fp8KvQuantize; } }
                "__tile_flash_attn_ext_pad_f32" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::FlashAttnExtPad; } }
                "__tile_flash_attn_ext_blk_f32" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::FlashAttnExtBlk; } }
                "__tile_indexer_score_one_direct_f32" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::Dsv4IndexerScoreOneDirect; } }
                "__tile_router_finalize_one_f32" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::Dsv4RouterFinalizeOne; } }
                "__tile_indexer_scores_tiled_f32" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::Dsv4IndexerScoresTiledF32; } }
                "__tile_indexer_scores_tiled_bf16" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::Dsv4IndexerScoresTiled; } }
                "__tile_indexed_mixed_attention_h8_f32" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::Dsv4IndexedMixedAttentionH8; } }
                "__tile_indexed_mixed_attention_h8_rb4_f32" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::Dsv4IndexedMixedAttentionH8Rb4; } }
                "__tile_flash_attn_ext_vec_reduce_f32" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::FlashAttnExtVecReduce; } }
                "__tile_flash_attn_ext_vec_setup_f32" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::FlashAttnExtVecSetup; } }
                "__tile_flash_attn_ext_vec_score_f32" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::FlashAttnExtVecScore; } }
                "__tile_flash_attn_ext_vec_out_f32"   => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::FlashAttnExtVecOut; } }
                "__tile_flash_attn_ext_vec_out_ms_f32" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::FlashAttnExtVecOutMS; } }
                "__tile_flash_attn_ext_setup_f32" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::FlashAttnExtSetup; } }
                "__tile_flash_attn_ext_score_f32" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::FlashAttnExtScore; } }
                "__tile_flash_attn_ext_out_f32" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::FlashAttnExtOut; } }
                "__tile_flash_attn_ext_out_ms_f32" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::FlashAttnExtOutMS; } }
                "__tile_dsv4_hc_expand_f32" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::Dsv4HcExpand; } }
                "__tile_dsv4_hc_expand4_f32" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::Dsv4HcExpand4; } }
                "__tile_dsv4_hc_weighted_sum_f32" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::Dsv4HcWeightedSum; } }
                "__tile_dsv4_hc_split_sinkhorn_hc4_f32" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::Dsv4HcSplitSinkhornHc4; } }
                "__tile_dsv4_hc_split_weighted_sum_hc4_f32" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::Dsv4HcSplitWeightedSumHc4; } }
                "__tile_dsv4_hc_split_weighted_sum_norm4_f32" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::Dsv4HcSplitWeightedSumNorm4; } }
                "__tile_argsort_f32_i32_desc" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::ArgsortF32I32Desc; } }
                "__tile_argsort_merge_f32_i32_desc" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::ArgsortMergeF32I32Desc; } }
                "__tile_argsort_f32_i32_desc_full" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::ArgsortF32I32DescFull; } }
                "__tile_argsort_merge_f32_i32_desc_full" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::ArgsortMergeF32I32DescFull; } }
                "__tile_moe_swiglu_weight_f32" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::Dsv4MoeSwigluWeight; } }
                "__tile_moe_swiglu_weight_f16" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::Dsv4MoeSwigluWeightF16; } }
                "__tile_mul_mm_id_map0_f32" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::Dsv4MulMmIdMap0; } }
                "__tile_mul_mm_id_map0_ne20_8_full" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::Dsv4MulMmIdMap0Ne20_8Full; } }
                "__tile_mul_mm_id_map0_ne20_4_full" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::Dsv4MulMmIdMap0Ne20_4Full; } }
                "__tile_mul_mm_id_map0_ne20_1_full" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::Dsv4MulMmIdMap0Ne20_1Full; } }
                "__tile_mul_mm_id_map0_ne20_2_full" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::Dsv4MulMmIdMap0Ne20_2Full; } }
                "__tile_mul_mm_id_map0_ne20_5_full" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::Dsv4MulMmIdMap0Ne20_5Full; } }
                "__tile_mul_mm_id_map0_ne20_6_full" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::Dsv4MulMmIdMap0Ne20_6Full; } }
                "__tile_mul_mm_id_map0_ne20_10_full" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::Dsv4MulMmIdMap0Ne20_10Full; } }
                "__tile_mul_mm_id_map0_ne20_16_full" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::Dsv4MulMmIdMap0Ne20_16Full; } }
                "__tile_mul_mm_id_map0_ne20_22_full" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::Dsv4MulMmIdMap0Ne20_22Full; } }
                "__tile_qkv_rms_norm_f32_4" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::Dsv4QkvRmsNormF32_4; } }
                "__tile_rms_norm_mul_f32_4" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::RmsNormMulF32_4; } }
                "__tile_rms_norm_f32_4" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::RmsNormF32_4; } }
                "__tile_softmax_f32_4_strided" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::SoftMaxF32_4; } }
                "__tile_softmax_f32_4_mask_f16" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::SoftMaxF32_4MaskF16; } }
                "__tile_softmax_f32_4_mask_f32" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::SoftMaxF32_4MaskF32; } }
                "__tile_softmax_f32_scalar_strided" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::SoftMaxF32Scalar; } }
                "__tile_softmax_f32_scalar_mask_f16" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::SoftMaxF32ScalarMaskF16; } }
                "__tile_softmax_f32_scalar_mask_f32" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::SoftMaxF32ScalarMaskF32; } }
                "__tile_softmax_f32_4_sink" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::SoftMaxF32_4Sink; } }
                "__tile_softmax_f32_scalar_sink" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::SoftMaxF32ScalarSink; } }
                "__tile_softmax_f32_4_mask_f16_sink" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::SoftMaxF32_4MaskF16Sink; } }
                "__tile_softmax_f32_4_mask_f32_sink" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::SoftMaxF32_4MaskF32Sink; } }
                "__tile_softmax_f32_scalar_mask_f16_sink" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::SoftMaxF32ScalarMaskF16Sink; } }
                "__tile_softmax_f32_scalar_mask_f32_sink" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::SoftMaxF32ScalarMaskF32Sink; } }
                "__tile_softmax_f32_4_alibi_f16"          => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::SoftMaxF32_4AlibiF16; } }
                "__tile_softmax_f32_4_alibi_f32"          => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::SoftMaxF32_4AlibiF32; } }
                "__tile_softmax_f32_4_alibi_f16_sink"     => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::SoftMaxF32_4AlibiF16Sink; } }
                "__tile_softmax_f32_4_alibi_f32_sink"     => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::SoftMaxF32_4AlibiF32Sink; } }
                "__tile_softmax_f32_scalar_alibi_f16"          => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::SoftMaxF32ScalarAlibiF16; } }
                "__tile_softmax_f32_scalar_alibi_f32"          => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::SoftMaxF32ScalarAlibiF32; } }
                "__tile_softmax_f32_scalar_alibi_f16_sink"     => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::SoftMaxF32ScalarAlibiF16Sink; } }
                "__tile_softmax_f32_scalar_alibi_f32_sink"     => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::SoftMaxF32ScalarAlibiF32Sink; } }
                "__tile_sum_rows_f32_strided"                  => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::SumRowsF32; } }
                "__tile_cpy_f32_f32_strided"                   => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::CpyF32F32; } }
                "__tile_cpy_f32_f16_strided"                   => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::CpyF32F16; } }
                "__tile_cpy_f16_f32_strided"                   => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::CpyF16F32; } }
                "__tile_concat_f32_strided"                    => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::ConcatF32; } }
                "__tile_repeat_f32_strided"                    => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::RepeatF32; } }
                "__tile_get_rows_f32_strided"                  => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::GetRowsF32; } }
                "__tile_get_rows_f16_strided"                  => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::GetRowsF16; } }
                "__tile_get_rows_i32_strided"                  => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::GetRowsI32; } }
                "__tile_set_rows_f32_i32_strided"              => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::SetRowsF32I32; } }
                "__tile_dsv4_softplus_sqrt_f32_4" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::Dsv4SoftplusSqrtF32_4; } }
                "__tile_sigmoid_f32_4" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::SigmoidF32_4; } }
                "__tile_relu_f32_4" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::ReluF32_4; } }
                "__tile_tanh_f32_4" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::TanhF32_4; } }
                "__tile_gelu_f32_4" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::GeluF32_4; } }
                "__tile_sqr_f32_4"  => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::SqrF32_4; } }
                "__tile_neg_f32_4"  => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::NegF32_4; } }
                "__tile_abs_f32_4"  => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::AbsF32_4; } }
                "__tile_step_f32_4" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::StepF32_4; } }
                "__tile_exp_f32_4"  => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::ExpF32_4; } }
                "__tile_log_f32_4"  => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::LogF32_4; } }
                "__tile_silu_f32_4" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::SiluF32_4; } }
                "__tile_hardsigmoid_f32_4" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::HardSigmoidF32_4; } }
                "__tile_hardswish_f32_4"   => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::HardSwishF32_4; } }
                "__tile_sigmoid_f16" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::SigmoidF16; } }
                "__tile_relu_f16"    => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::ReluF16; } }
                "__tile_tanh_f16"    => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::TanhF16; } }
                "__tile_gelu_f16"    => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::GeluF16; } }
                "__tile_silu_f16"    => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::SiluF16; } }
                "__tile_hardsigmoid_f16" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::HardSigmoidF16; } }
                "__tile_hardswish_f16"   => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::HardSwishF16; } }
                "__tile_sqr_f16"  => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::SqrF16; } }
                "__tile_neg_f16"  => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::NegF16; } }
                "__tile_abs_f16"  => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::AbsF16; } }
                "__tile_step_f16" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::StepF16; } }
                "__tile_exp_f16"  => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::ExpF16; } }
                "__tile_log_f16"  => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::LogF16; } }
                "__tile_sigmoid_f32_scalar" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::SigmoidF32Scalar; } }
                "__tile_relu_f32_scalar"    => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::ReluF32Scalar; } }
                "__tile_tanh_f32_scalar"    => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::TanhF32Scalar; } }
                "__tile_gelu_f32_scalar"    => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::GeluF32Scalar; } }
                "__tile_silu_f32_scalar"    => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::SiluF32Scalar; } }
                "__tile_hardsigmoid_f32_scalar" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::HardSigmoidF32Scalar; } }
                "__tile_hardswish_f32_scalar"   => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::HardSwishF32Scalar; } }
                "__tile_sqr_f32_scalar"  => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::SqrF32Scalar; } }
                "__tile_neg_f32_scalar"  => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::NegF32Scalar; } }
                "__tile_abs_f32_scalar"  => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::AbsF32Scalar; } }
                "__tile_step_f32_scalar" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::StepF32Scalar; } }
                "__tile_exp_f32_scalar"  => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::ExpF32Scalar; } }
                "__tile_log_f32_scalar"  => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::LogF32Scalar; } }
                "__tile_swiglu_f32" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::SwigluF32; } }
                "__tile_mul_mv_f32_f32_short" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::MulMvF32F32Short; } }
                "__tile_mul_mv_f32_f32_setup" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::MulMvF32F32Setup; } }
                "__tile_mul_mv_f32_f32_acc" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::MulMvF32F32Acc; } }
                "__tile_mul_mv_f32_f32_reduce" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::MulMvF32F32Reduce; } }
                "__tile_mul_mv_f16_f32_reduce" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::MulMvF16F32Reduce; } }
                "__tile_mul_mv_f32_f32_4_reduce" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::MulMvF32F32_4Reduce; } }
                "__tile_mul_mv_f16_f32_4_reduce" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::MulMvF16F32_4Reduce; } }
                "__tile_mul_mv_f16_f32_pair_4" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::MulMvF16F32Pair_4; } }
                "__tile_mul_mv_q8_0_f32" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::MulMvQ8_0F32; } }
                "__tile_mul_mv_id_q8_0_f32" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::MulMvIdQ8_0F32; } }
                "__tile_mul_mv_id_q2_K_f32" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::MulMvIdQ2KF32; } }
                "__tile_mul_mv_id_q4_K_f32" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::MulMvIdQ4KF32; } }
                "__tile_mul_mv_id_iq2_xxs_f32" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::MulMvIdIq2XxsF32; } }
                "__tile_mul_mv_id_iq2_xxs_pair_f32" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::MulMvIdIq2XxsPairF32; } }
                ...
                "__tile_dsv4_shared_gate_up_swiglu_q8_0" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::Dsv4SharedGateUpSwigluQ8_0; } }
                "__tile_mul_mv_f32_f32_4" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::MulMvF32F32_4; } }
                "__tile_mul_mv_f16_f32_4" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::MulMvF16F32_4; } }
                "__tile_soft_max_f32" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::SoftMaxFullF32; } }
                "__tile_soft_max_f32_4" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::SoftMaxFullF32_4; } }
                "__tile_bin_fuse_f32" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::BinFuseF32F32F32; } }
                "__tile_unary_f32" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::UnaryF32F32; } }
                "__tile_unary_f32_4" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::UnaryF32F32_4; } }
                "__tile_unary_f16" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::UnaryF16F16; } }
                "__tile_dsv4_rope_tail_f32" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::Dsv4RopeTailF32; } }
                "__tile_flash_attn_ext_f16_dk512_dv512" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::FlashAttnExtF16Dk512Dv512; } }
                "__tile_flash_attn_ext_vec_f16_dk512_dv512" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::FlashAttnExtVecF16Dk512Dv512; } }
                "__tile_dsv4_topk_mask_f32" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::Dsv4TopkMask; } }
                "__tile_dsv4_q8_hc_expand4_q8_0" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::Dsv4Q8HcExpand4Q8_0; } }
                "__tile_dsv4_shared_down_hc_expand4_q8_0" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::Dsv4SharedDownHcExpand4Q8_0; } }
                "__tile_mul_mv_f32_f32" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::MulMvF32F32; } }
                "__tile_mul_mv_f16_f32" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::MulMvF16F32; } }
                "__tile_mul_mv_ext_f16_f32_r1_2" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::MulMvExtF16F32R1_2; } }
                "__tile_mul_mv_ext_f16_f32_r1_3" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::MulMvExtF16F32R1_3; } }
                "__tile_mul_mv_ext_f16_f32_r1_4" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::MulMvExtF16F32R1_4; } }
                "__tile_mul_mv_ext_f16_f32_r1_5" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::MulMvExtF16F32R1_5; } }
                "__tile_mul_mv_ext_q8_0_f32_r1_2" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::MulMvExtQ8_0F32R1_2; } }
                "__tile_mul_mv_ext_q8_0_f32_r1_3" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::MulMvExtQ8_0F32R1_3; } }
                "__tile_mul_mv_ext_q8_0_f32_r1_4" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::MulMvExtQ8_0F32R1_4; } }
                "__tile_mul_mv_ext_q8_0_f32_r1_5" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::MulMvExtQ8_0F32R1_5; } }
                "__tile_mul_mm_f16_f32" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::MulMmF16F32; } }
                "__tile_mul_mm_q8_0_f32" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::MulMmQ8_0F32; } }
                "__tile_mul_mm_id_q8_0_f32" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::MulMmIdQ8_0F32; } }
                "__tile_mul_mm_id_q8_0_f16" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::MulMmIdQ8_0F16; } }
                "__tile_mul_mm_id_q2_K_f32" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::MulMmIdQ2KF32; } }
                "__tile_mul_mm_id_q2_K_f16" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::MulMmIdQ2KF16; } }
                "__tile_mul_mm_id_q4_K_f32" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::MulMmIdQ4KF32; } }
                "__tile_mul_mm_id_q4_K_f16" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::MulMmIdQ4KF16; } }
                "__tile_mul_mm_id_iq2_xxs_f32" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::MulMmIdIq2XxsF32; } }
                "__tile_mul_mm_id_iq2_xxs_f16" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::MulMmIdIq2XxsF16; } }
                "__tile_mul_mv_f16_f32_short" => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::MulMvF16F32Short; } }
                "__tile_embedding_f32"     => { ctx.kernel_type = KernelType::Embedding; }
                "__tile_causal_mask_f32"   => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::CausalMask; } }
                // LLM inference ops
                "__tile_silu_f32"          => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::SiLU; } }
                "__tile_cast_bf16_f32"     => { if ctx.kernel_type == KernelType::Copy { ctx.kernel_type = KernelType::CastBf16F32; } }
                "__tile_matmul_transposed_f32" => { ctx.kernel_type = KernelType::MatmulTransposed; }
...
module {
  llvm.func @softmax_f16(%arg0: !llvm.ptr<1>, %arg1: !llvm.ptr<1>) attributes {hacc.entry} {
    ...
    %4 = llvm.call @__tile_softmax_f16(%3, %3, %1, %2) : (i32, i32, i32, i32) -> i32
...
module {
  llvm.func @layernorm(%arg0: !llvm.ptr<1>, %arg1: !llvm.ptr<1>, %arg2: !llvm.ptr<1>, %arg3: !llvm.ptr<1>) attributes {hacc.entry} {
    ...
    %a = llvm.call @__tile_load_f32(%arg0, %r, %c) : (!llvm.ptr<1>, i32, i32) -> i32
    %g = llvm.call @__tile_load_f32(%arg1, %r, %c) : (!llvm.ptr<1>, i32, i32) -> i32
    %b = llvm.call @__tile_load_f32(%arg2, %r, %c) : (!llvm.ptr<1>, i32, i32) -> i32
    %res = llvm.call @__tile_layernorm_f32(%a, %g, %b, %r, %c) : (i32, i32, i32, i32, i32) -> i32
...
module {
  llvm.func @vq_l2dist(%arg0: !llvm.ptr<1>, %arg1: !llvm.ptr<1>, %arg2: !llvm.ptr<1>) attributes {hacc.entry} {
    ...
    %a = llvm.call @__tile_load_f32(%arg0, %r, %c) : (!llvm.ptr<1>, i32, i32) -> i32
    %b = llvm.call @__tile_load_f32(%arg1, %r, %c) : (!llvm.ptr<1>, i32, i32) -> i32
    %res = llvm.call @__tile_l2dist_f32(%a, %b, %r, %c) : (i32, i32, i32, i32) -> i32
    llvm.call @__tile_store_f32(%arg2, %res, %r, %c) : (!llvm.ptr<1>, i32, i32, i32) -> ()
    llvm.return
  }
}
...
module {
  llvm.func @ema_scatter(%arg0: !llvm.ptr<1>, %arg1: !llvm.ptr<1>, %arg2: !llvm.ptr<1>, %arg3: !llvm.ptr<1>) attributes {hacc.entry} {
    ...
    %a = llvm.call @__tile_load_f32(%arg0, %r, %c) : (!llvm.ptr<1>, i32, i32) -> i32
    %b = llvm.call @__tile_load_f32(%arg1, %r, %c) : (!llvm.ptr<1>, i32, i32) -> i32
    %res = llvm.call @__tile_scatter_add_f32(%a, %b, %r, %c) : (i32, i32, i32, i32) -> i32
    llvm.call @__tile_store_f32(%arg2, %res, %r, %c) : (!llvm.ptr<1>, i32, i32, i32) -> ()
    llvm.return
  }
}
...
module {
  llvm.func @vec_clamp(%arg0: !llvm.ptr<1>, %arg1: !llvm.ptr<1>) attributes {hacc.entry} {
    ...
    %c = llvm.mlir.constant(256 : i32) : i32
    %a = llvm.call @__tile_load_f32(%arg0, %r, %c) : (!llvm.ptr<1>, i32, i32) -> i32
    %res = llvm.call @__tile_clamp_f32(%a, %a, %r, %c, %r, %c) : (i32, i32, i32, i32, i32, i32) -> i32
    llvm.call @__tile_store_f32(%arg1, %res, %r, %c) : (!llvm.ptr<1>, i32, i32, i32) -> ()
    llvm.return
  }
}
...
module {
  llvm.func @tile_slice(%arg0: !llvm.ptr<1>, %arg1: !llvm.ptr<1>) attributes {hacc.entry} {
    ...
    %c = llvm.mlir.constant(256 : i32) : i32
    %a = llvm.call @__tile_load_f32(%arg0, %r, %c) : (!llvm.ptr<1>, i32, i32) -> i32
    %res = llvm.call @__tile_slice_f32(%a, %a, %r, %c, %c, %r) : (i32, i32, i32, i32, i32, i32) -> i32
    llvm.call @__tile_store_f32(%arg1, %res, %r, %c) : (!llvm.ptr<1>, i32, i32, i32) -> ()
    llvm.return
  }
}
...
module {
  llvm.func @tile_concat(%arg0: !llvm.ptr<1>, %arg1: !llvm.ptr<1>, %arg2: !llvm.ptr<1>) attributes {hacc.entry} {
    ...
    %a = llvm.call @__tile_load_f32(%arg0, %r, %c) : (!llvm.ptr<1>, i32, i32) -> i32
    %b = llvm.call @__tile_load_f32(%arg1, %r, %c) : (!llvm.ptr<1>, i32, i32) -> i32
    %res = llvm.call @__tile_concat_f32(%a, %b, %r, %c) : (i32, i32, i32, i32) -> i32
    llvm.call @__tile_store_f32(%arg2, %res, %r, %c) : (!llvm.ptr<1>, i32, i32, i32) -> ()
    llvm.return
  }
}
...
module {
  llvm.func @tile_scatter(%arg0: !llvm.ptr<1>, %arg1: !llvm.ptr<1>, %arg2: !llvm.ptr<1>) attributes {hacc.entry} {
    ...
    %a = llvm.call @__tile_load_f32(%arg0, %r, %c) : (!llvm.ptr<1>, i32, i32) -> i32
    %b = llvm.call @__tile_load_f32(%arg1, %r, %c) : (!llvm.ptr<1>, i32, i32) -> i32
    %res = llvm.call @__tile_scatter_f32(%a, %b, %r, %c) : (i32, i32, i32, i32) -> i32
    llvm.call @__tile_store_f32(%arg2, %res, %r, %c) : (!llvm.ptr<1>, i32, i32, i32) -> ()
    llvm.return
  }
}
...
module {
  llvm.func @tile_gather(%arg0: !llvm.ptr<1>, %arg1: !llvm.ptr<1>, %arg2: !llvm.ptr<1>) attributes {hacc.entry} {
    ...
    %a = llvm.call @__tile_load_f32(%arg0, %r, %c) : (!llvm.ptr<1>, i32, i32) -> i32
    %b = llvm.call @__tile_load_f32(%arg1, %r, %c) : (!llvm.ptr<1>, i32, i32) -> i32
    %res = llvm.call @__tile_gather_f32(%a, %b, %r, %c) : (i32, i32, i32, i32) -> i32
    llvm.call @__tile_store_f32(%arg2, %res, %r, %c) : (!llvm.ptr<1>, i32, i32, i32) -> ()
    llvm.return
  }
}
...
module {
  llvm.func @tile_topk(%arg0: !llvm.ptr<1>, %arg1: !llvm.ptr<1>, %arg2: !llvm.ptr<1>) attributes {hacc.entry} {
    ...
    %a = llvm.call @__tile_load_f32(%arg0, %r, %c) : (!llvm.ptr<1>, i32, i32) -> i32
    %b = llvm.call @__tile_load_f32(%arg1, %r, %c) : (!llvm.ptr<1>, i32, i32) -> i32
    %res = llvm.call @__tile_topk_f32(%a, %b, %r, %c) : (i32, i32, i32, i32) -> i32
    llvm.call @__tile_store_f32(%arg2, %res, %r, %c) : (!llvm.ptr<1>, i32, i32, i32) -> ()
    llvm.return
  }
}
...
module {
  llvm.func @tile_matmul(%arg0: !llvm.ptr<1>, %arg1: !llvm.ptr<1>, %arg2: !llvm.ptr<1>) attributes {hacc.entry} {
    ...
    %a = llvm.call @__tile_load_f16(%arg0, %r, %c) : (!llvm.ptr<1>, i32, i32) -> i32
    %b = llvm.call @__tile_load_f16(%arg1, %r, %c) : (!llvm.ptr<1>, i32, i32) -> i32
    %res = llvm.call @__tile_matmul_f16(%a, %b, %r, %c, %c) : (i32, i32, i32, i32, i32) -> i32
    llvm.call @__tile_store_f16(%arg2, %res, %r, %c) : (!llvm.ptr<1>, i32, i32, i32) -> ()
    llvm.return
  }
}
"#
...
module {
  llvm.func @gqa_test(%arg0: !llvm.ptr<1>, %arg1: !llvm.ptr<1>, %arg2: !llvm.ptr<1>, %arg3: !llvm.ptr<1>) attributes {hacc.entry} {
    ...
    %res = llvm.call @__tile_attention_gqa_f32(%q, %k, %v, %seq, %dim, %nh, %nkv, %causal) : (i32, i32, i32, i32, i32, i32, i32, i32) -> i32
...
module {
  llvm.func @gqa_params(%arg0: !llvm.ptr<1>, %arg1: !llvm.ptr<1>, %arg2: !llvm.ptr<1>, %arg3: !llvm.ptr<1>) attributes {hacc.entry} {
    ...
    %k = llvm.call @__tile_load_f32(%arg1, %nkv, %dim) : (!llvm.ptr<1>, i32, i32) -> i32
    %v = llvm.call @__tile_load_f32(%arg2, %nkv, %dim) : (!llvm.ptr<1>, i32, i32) -> i32
    %res = llvm.call @__tile_attention_gqa_f32(%q, %k, %v, %seq, %dim, %nh, %nkv, %c) : (i32, i32, i32, i32, i32, i32, i32, i32) -> i32
    llvm.call @__tile_store_f32(%arg3, %res, %nh, %dim) : (!llvm.ptr<1>, i32, i32, i32) -> ()
    llvm.return
  }
}
...
module {
  llvm.func @matvec_f16(%arg0: !llvm.ptr<1>, %arg1: !llvm.ptr<1>, %arg2: !llvm.ptr<1>) attributes {hacc.entry} {
    ...
    %a = llvm.call @__tile_load_f32(%arg0, %r, %c) : (!llvm.ptr<1>, i32, i32) -> i32
    %b = llvm.call @__tile_load_f32(%arg1, %r, %c) : (!llvm.ptr<1>, i32, i32) -> i32
    %res = llvm.call @__tile_matvec_f16(%a, %b, %r, %c) : (i32, i32, i32, i32) -> i32
    llvm.call @__tile_store_f32(%arg2, %res, %r, %c) : (!llvm.ptr<1>, i32, i32, i32) -> ()
    llvm.return
  }
}
...
module {
  llvm.func @rope_inplace(%arg0: !llvm.ptr<1>) attributes {hacc.entry} {
    ...
    %c = llvm.mlir.constant(128 : i32) : i32
    %a = llvm.call @__tile_load_f32(%arg0, %r, %c) : (!llvm.ptr<1>, i32, i32) -> i32
    %res = llvm.call @__tile_rope_inplace_f32(%a, %a, %r, %c) : (i32, i32, i32, i32) -> i32
    llvm.call @__tile_store_f32(%arg0, %res, %r, %c) : (!llvm.ptr<1>, i32, i32, i32) -> ()
    llvm.return
  }
}
...
module {
  llvm.func @kv_cache_update(%arg0: !llvm.ptr<1>, %arg1: !llvm.ptr<1>) attributes {hacc.entry} {
    ...
    %c = llvm.mlir.constant(128 : i32) : i32
    %a = llvm.call @__tile_load_f32(%arg0, %r, %c) : (!llvm.ptr<1>, i32, i32) -> i32
    %res = llvm.call @__tile_kv_cache_update_f32(%a, %a, %r, %c) : (i32, i32, i32, i32) -> i32
    llvm.call @__tile_store_f32(%arg1, %res, %r, %c) : (!llvm.ptr<1>, i32, i32, i32) -> ()
    llvm.return
  }
}
...
module {
  llvm.func @attention_decode(%arg0: !llvm.ptr<1>, %arg1: !llvm.ptr<1>, %arg2: !llvm.ptr<1>, %arg3: !llvm.ptr<1>) attributes {hacc.entry} {
    ...
    %res = llvm.call @__tile_attention_decode_f32(%q, %k, %v, %r, %c) : (i32, i32, i32, i32, i32) -> i32
    llvm.call @__tile_store_f32(%arg3, %res, %r, %c) : (!llvm.ptr<1>, i32, i32, i32) -> ()
    llvm.return
  }
}
...
module {
  llvm.func @rms_norm(%arg0: !llvm.ptr<1>, %arg1: !llvm.ptr<1>) attributes {hacc.entry} {
    ...
    %c = llvm.mlir.constant(1536 : i32) : i32
    %a = llvm.call @__tile_load_f32(%arg0, %r, %c) : (!llvm.ptr<1>, i32, i32) -> i32
    %res = llvm.call @__tile_rms_norm_f32(%a, %a, %r, %c) : (i32, i32, i32, i32) -> i32
    llvm.call @__tile_store_f32(%arg1, %res, %r, %c) : (!llvm.ptr<1>, i32, i32, i32) -> ()
    llvm.return
  }
}
...
module {
  llvm.func @sk(%arg0: !llvm.ptr<1>, %arg1: !llvm.ptr<1>) attributes {hacc.entry} {
    ...
    %a = llvm.call @__tile_load_f32(%arg0, %r, %c) : (!llvm.ptr<1>, i32, i32) -> i32
    %y = llvm.call @__tile_scale_f32(%a, %a, %s, %r, %c) : (i32, i32, f32, i32, i32) -> i32
...
module {
  llvm.func @uk(%arg0: !llvm.ptr<1>, %arg1: !llvm.ptr<1>) attributes {hacc.entry} {
    ...
    %c = llvm.mlir.constant(256 : i32) : i32
    %a = llvm.call @__tile_load_f32(%arg0, %r, %c) : (!llvm.ptr<1>, i32, i32) -> i32
    %y = llvm.call @__tile_sqrt_f32(%a, %a, %r, %c) : (i32, i32, i32, i32) -> i32
    llvm.call @__tile_store_f32(%arg1, %y, %r, %c) : (!llvm.ptr<1>, i32, i32, i32) -> ()
    llvm.return
  }
}
...
module {
  llvm.func @uk(%arg0: !llvm.ptr<1>, %arg1: !llvm.ptr<1>) attributes {hacc.entry} {
    ...
    %c = llvm.mlir.constant(256 : i32) : i32
    %a = llvm.call @__tile_load_f32(%arg0, %r, %c) : (!llvm.ptr<1>, i32, i32) -> i32
    %y = llvm.call @__tile_softplus_f32(%a, %a, %r, %c) : (i32, i32, i32, i32) -> i32
    llvm.call @__tile_store_f32(%arg1, %y, %r, %c) : (!llvm.ptr<1>, i32, i32, i32) -> ()
    llvm.return
  }
}
...
module {
  llvm.func @wk(%arg0: !llvm.ptr<1>, %arg1: !llvm.ptr<1>, %arg2: !llvm.ptr<1>, %arg3: !llvm.ptr<1>) attributes {hacc.entry} {
    ...
    %r = llvm.mlir.constant(1 : i32) : i32
    %c = llvm.mlir.constant(256 : i32) : i32
    %cd = llvm.call @__tile_load_f32(%arg0, %r, %c) : (!llvm.ptr<1>, i32, i32) -> i32
    %a = llvm.call @__tile_load_f32(%arg1, %r, %c) : (!llvm.ptr<1>, i32, i32) -> i32
    %b = llvm.call @__tile_load_f32(%arg2, %r, %c) : (!llvm.ptr<1>, i32, i32) -> i32
    %y = llvm.call @__tile_where_f32(%cd, %cd, %a, %b, %r, %c) : (i32, i32, i32, i32, i32, i32) -> i32
...
module {
  llvm.func @amk(%arg0: !llvm.ptr<1>, %arg1: !llvm.ptr<1>) attributes {hacc.entry} {
    ...
    %c = llvm.mlir.constant(64 : i32) : i32
    %a = llvm.call @__tile_load_f32(%arg0, %r, %c) : (!llvm.ptr<1>, i32, i32) -> i32
    %y = llvm.call @__tile_argmin_f32(%a, %a, %r, %c) : (i32, i32, i32, i32) -> i32
    llvm.call @__tile_store_f32(%arg1, %y, %r, %c) : (!llvm.ptr<1>, i32, i32, i32) -> ()
    llvm.return
  }
}
...
module {
  llvm.func @uk(%arg0: !llvm.ptr<1>, %arg1: !llvm.ptr<1>) attributes {hacc.entry} {
    ...
    %c = llvm.mlir.constant(256 : i32) : i32
    %a = llvm.call @__tile_load_f32(%arg0, %r, %c) : (!llvm.ptr<1>, i32, i32) -> i32
    %y = llvm.call @__tile_repeat_f32(%a, %a, %r, %c) : (i32, i32, i32, i32) -> i32
    llvm.call @__tile_store_f32(%arg1, %y, %r, %c) : (!llvm.ptr<1>, i32, i32, i32) -> ()
    llvm.return
  }
}
...
module {
  llvm.func @uk(%arg0: !llvm.ptr<1>, %arg1: !llvm.ptr<1>) attributes {hacc.entry} {
    ...
    %c = llvm.mlir.constant(256 : i32) : i32
    %a = llvm.call @__tile_load_f32(%arg0, %r, %c) : (!llvm.ptr<1>, i32, i32) -> i32
    %y = llvm.call @__tile_sum_rows_f32(%a, %a, %r, %c) : (i32, i32, i32, i32) -> i32
    llvm.call @__tile_store_f32(%arg1, %y, %r, %c) : (!llvm.ptr<1>, i32, i32, i32) -> ()
    llvm.return
  }
}
...
module {
  llvm.func @grk(%arg0: !llvm.ptr<1>, %arg1: !llvm.ptr<1>, %arg2: !llvm.ptr<1>) attributes {hacc.entry} {
    ...
    %c = llvm.mlir.constant(256 : i32) : i32
    %a = llvm.call @__tile_load_f32(%arg0, %r, %c) : (!llvm.ptr<1>, i32, i32) -> i32
    %y = llvm.call @__tile_get_rows_f32(%a, %a, %arg1, %r, %c) : (i32, i32, !llvm.ptr<1>, i32, i32) -> i32
    llvm.call @__tile_store_f32(%arg2, %y, %r, %c) : (!llvm.ptr<1>, i32, i32, i32) -> ()
    llvm.return
  }
}
...
module {
  llvm.func @srk(%arg0: !llvm.ptr<1>, %arg1: !llvm.ptr<1>, %arg2: !llvm.ptr<1>) attributes {hacc.entry} {
    ...
    %c = llvm.mlir.constant(256 : i32) : i32
    %a = llvm.call @__tile_load_f32(%arg0, %r, %c) : (!llvm.ptr<1>, i32, i32) -> i32
    %y = llvm.call @__tile_set_rows_f32(%a, %a, %arg1, %r, %c) : (i32, i32, !llvm.ptr<1>, i32, i32) -> i32
    llvm.call @__tile_store_f32(%arg2, %y, %r, %c) : (!llvm.ptr<1>, i32, i32, i32) -> ()
    llvm.return
  }
}
