...
//! |---|---|
//! | `ascend_tile_load_f32(gm, rows, cols)` | `ref[...]` (full load) or `ref[r_off:r_off+R, :]` |
//! | `ascend_tile_store_f32(gm, buf, rows, cols)` | `ref[...] = tile` or `ref[r_off:r_off+R, :] = tile` |
//! | `ascend_tile_add_f32(0, a, b, rows, cols)` | `a + b` |
//! | `ascend_tile_sub_f32(0, a, b, rows, cols)` | `a - b` |
//! | `ascend_tile_mul_f32(0, a, b, rows, cols)` | `a * b` |
//! | `ascend_tile_exp_f32(0, src, rows, cols)` | `jnp.exp(src)` |
//! | `ascend_tile_softmax_f32(0, src, rows, cols)` | 4-step: max → sub → exp → sum → div |
//! | `ascend_tile_matmul_f32(0, a, b, m, k, n)` | `jnp.dot(a, b)` |
//! | `ascend_tile_reduce_max_f32(0, src, rows, cols)` | `jnp.max(src, axis=-1, keepdims=True)` |
//! | `ascend_tile_reduce_sum_f32(0, src, rows, cols)` | `jnp.sum(src, axis=-1, keepdims=True)` |
//! | `ascend_tile_scale_f32(0, src, scalar, rows, cols)` | `src * scalar` |
//! | `ascend_tile_transpose_f32(0, src, rows, cols)` | `jnp.transpose(src)` |
//! | `ascend_tile_rsqrt_f32(0, src, rows, cols)` | `jax.lax.rsqrt(src)` |
//! | `ascend_tile_log_f32(0, src, rows, cols)` | `jnp.log(src)` |
//! | `ascend_tile_sigmoid_f32(0, src, rows, cols)` | `jax.nn.sigmoid(src)` |
//! | `ascend_tile_silu_f32(0, src, rows, cols)` | `jax.nn.silu(src)` |
//! | `ascend_tile_clamp_f32(0, src, min, max, rows, cols)` | `jnp.clip(src, min, max)` |
//! | `ascend_tile_cast_f32_f16(0, src, rows, cols)` | `src.astype(jnp.float16)` |
//! | `ascend_tile_cast_f16_f32(0, src, rows, cols)` | `src.astype(jnp.float32)` |
//! | `ascend_tile_slice_f32(0, src, r_off, c_off, sr, sc, dr, dc)` | `src[r_off:r_off+dr, c_off:c_off+dc]` |
//! | `ascend_tile_concat_f32(0, a, b, rows, cols_a, cols_b)` | `jnp.concatenate((a, b), axis=1)` |
//! | `ascend_tile_rope_f32(0, src, pos, rows, cols)` | Rotary position embedding (vectorised) |
//! | `ascend_tile_fill_f32(dst, scalar, rows, cols)` | `jnp.full((rows, cols), scalar)` |
//!
...
fn translate_call(
    ...
    match callee.as_str() {
        ...
        // ── ascend_tile_token_accept_f32(dst, draft, target, probs, threshold, rows) → u32 ──
...
fn translate_cast(
    ...
    if args.len() < 4 {
        return Err(format!("ascend_tile_cast_*: expected 4 args, got {}", args.len()));
    }
    let src_ssa = args[1].trim();
    let (rows, cols) = ctx
        .get_tile_shape(src_ssa)
...
module {
  llvm.func @matmul_test(%arg0: !llvm.ptr<1>, %arg1: !llvm.ptr<1>, %arg2: !llvm.ptr<1>) attributes {hacc.entry} {
    ...
    %res = llvm.call @ascend_tile_matmul_f32(%a, %a, %b, %m, %k, %n) : (i32, i32, i32, i32, i32, i32) -> i32
...
module {
  llvm.func @tile_conv1d(%arg0: !llvm.ptr<1>, %arg1: !llvm.ptr<1>) attributes {hacc.entry} {
    ...
    %x = llvm.call @ascend_tile_load_f32(%arg0, %r, %c) : (!llvm.ptr<1>, i32, i32) -> i32
    %s1 = llvm.call @ascend_tile_scale_f32(%x, %x, %w1, %r, %c) : (i32, i32, f32, i32, i32) -> i32
    %l = llvm.call @ascend_tile_load_f32(%arg0, %r, %c) : (!llvm.ptr<1>, i32, i32) -> i32
    %s0 = llvm.call @ascend_tile_scale_f32(%l, %l, %w0, %r, %c) : (i32, i32, f32, i32, i32) -> i32
    %a1 = llvm.call @ascend_tile_add_f32(%s1, %s1, %s0, %r, %c) : (i32, i32, i32, i32, i32) -> i32
    %rr = llvm.call @ascend_tile_load_f32(%arg0, %r, %c) : (!llvm.ptr<1>, i32, i32) -> i32
    %s2 = llvm.call @ascend_tile_scale_f32(%rr, %rr, %w0, %r, %c) : (i32, i32, f32, i32, i32) -> i32
    %a2 = llvm.call @ascend_tile_add_f32(%a1, %a1, %s2, %r, %c) : (i32, i32, i32, i32, i32) -> i32
    %cl = llvm.call @ascend_tile_clamp_f32(%a2, %a2, %lo, %hi, %r, %c) : (i32, i32, f32, f32, i32, i32) -> i32
...
module {
  llvm.func @tile_rope(%arg0: !llvm.ptr<1>, %arg1: !llvm.ptr<1>) attributes {hacc.entry} {
    ...
    %x = llvm.call @ascend_tile_load_f32(%arg0, %r, %c) : (!llvm.ptr<1>, i32, i32) -> i32
    %y = llvm.call @ascend_tile_rope_f32(%x, %x, %pos, %r, %c) : (i32, i32, i32, i32, i32) -> i32
...
module {
  llvm.func @op_test(%arg0: !llvm.ptr<1>, %arg1: !llvm.ptr<1>, %arg2: !llvm.ptr<1>) attributes {hacc.entry} {
    ...
    %a = llvm.call @ascend_tile_load_f32(%arg0, %r, %c) : (!llvm.ptr<1>, i32, i32) -> i32
    %b = llvm.call @ascend_tile_load_f32(%arg1, %r, %c) : (!llvm.ptr<1>, i32, i32) -> i32
    %res = llvm.call @ascend_tile_sub_f32(%a, %a, %b, %r, %c) : (i32, i32, i32, i32, i32) -> i32
    llvm.call @ascend_tile_store_f32(%arg2, %res, %r, %c) : (!llvm.ptr<1>, i32, i32, i32) -> ()
    llvm.return
  }
}
...
module {
  llvm.func @op_test(%arg0: !llvm.ptr<1>, %arg1: !llvm.ptr<1>, %arg2: !llvm.ptr<1>) attributes {hacc.entry} {
    ...
    %a = llvm.call @ascend_tile_load_f32(%arg0, %r, %c) : (!llvm.ptr<1>, i32, i32) -> i32
    %b = llvm.call @ascend_tile_load_f32(%arg1, %r, %c) : (!llvm.ptr<1>, i32, i32) -> i32
    %res = llvm.call @ascend_tile_mul_f32(%a, %a, %b, %r, %c) : (i32, i32, i32, i32, i32) -> i32
    llvm.call @ascend_tile_store_f32(%arg2, %res, %r, %c) : (!llvm.ptr<1>, i32, i32, i32) -> ()
    llvm.return
  }
}
...
module {
  llvm.func @op_test(%arg0: !llvm.ptr<1>, %arg1: !llvm.ptr<1>) attributes {hacc.entry} {
    ...
    %a = llvm.call @ascend_tile_load_f32(%arg0, %r, %c) : (!llvm.ptr<1>, i32, i32) -> i32
    %res = llvm.call @ascend_tile_scale_f32(%a, %a, %s, %r, %c) : (i32, i32, f32, i32, i32) -> i32
    llvm.call @ascend_tile_store_f32(%arg1, %res, %r, %c) : (!llvm.ptr<1>, i32, i32, i32) -> ()
    llvm.return
  }
}
...
module {
  llvm.func @op_test(%arg0: !llvm.ptr<1>, %arg1: !llvm.ptr<1>) attributes {hacc.entry} {
    ...
    %a = llvm.call @ascend_tile_load_f32(%arg0, %r, %c) : (!llvm.ptr<1>, i32, i32) -> i32
    %res = llvm.call @ascend_tile_clamp_f32(%a, %a, %lo, %hi, %r, %c) : (i32, i32, f32, f32, i32, i32) -> i32
    llvm.call @ascend_tile_store_f32(%arg1, %res, %r, %c) : (!llvm.ptr<1>, i32, i32, i32) -> ()
    llvm.return
  }
}
...
module {
  llvm.func @verify_test(%arg0: !llvm.ptr<1>, %arg1: !llvm.ptr<1>, %arg2: !llvm.ptr<1>) attributes {hacc.entry} {
    ...
    %res = llvm.call @ascend_tile_draft_verify_f32(%draft, %draft, %tgt, %r, %c) : (i32, i32, i32, i32, i32) -> i32
    llvm.call @ascend_tile_store_f32(%arg2, %res, %r, %c) : (!llvm.ptr<1>, i32, i32, i32) -> ()
    llvm.return
  }
}
...
module {
  llvm.func @attn_test(%arg0: !llvm.ptr<1>, %arg1: !llvm.ptr<1>, %arg2: !llvm.ptr<1>, %arg3: !llvm.ptr<1>) attributes {hacc.entry} {
    ...
    %q = llvm.call @ascend_tile_load_f32(%arg0, %h, %hd) : (!llvm.ptr<1>, i32, i32) -> i32
    %k = llvm.call @ascend_tile_load_f32(%arg1, %h, %hd) : (!llvm.ptr<1>, i32, i32) -> i32
    %v = llvm.call @ascend_tile_load_f32(%arg2, %h, %hd) : (!llvm.ptr<1>, i32, i32) -> i32
    %res = llvm.call @ascend_tile_attention_gqa_f32(%q, %q, %k, %v, %h, %hd, %sl, %kvh) : (i32, i32, i32, i32, i32, i32, i32, i32) -> i32
    llvm.call @ascend_tile_store_f32(%arg3, %res, %h, %hd) : (!llvm.ptr<1>, i32, i32, i32) -> ()
    llvm.return
  }
}
"#;
...
module {
  llvm.func @rms_test(%arg0: !llvm.ptr<1>, %arg1: !llvm.ptr<1>) attributes {hacc.entry} {
    ...
    %a = llvm.call @ascend_tile_load_f32(%arg0, %r, %c) : (!llvm.ptr<1>, i32, i32) -> i32
    %res = llvm.call @ascend_tile_rms_norm_f32(%a, %a, %e, %r, %c) : (i32, i32, i32, i32, i32) -> i32
    llvm.call @ascend_tile_store_f32(%arg1, %res, %r, %c) : (!llvm.ptr<1>, i32, i32, i32) -> ()
    llvm.return
  }
}
...
module {
  llvm.func @quant_test(%arg0: !llvm.ptr<1>, %arg1: !llvm.ptr<1>, %arg2: !llvm.ptr<1>) attributes {hacc.entry} {
    ...
    %c = llvm.mlir.constant(64 : i32) : i32
    %a = llvm.call @ascend_tile_load_f32(%arg0, %r, %c) : (!llvm.ptr<1>, i32, i32) -> i32
    %s = llvm.call @ascend_tile_load_f32(%arg1, %r, %c) : (!llvm.ptr<1>, i32, i32) -> i32
...
module {
  llvm.func @gather_test(%arg0: !llvm.ptr<1>, %arg1: !llvm.ptr<1>, %arg2: !llvm.ptr<1>) attributes {hacc.entry} {
    ...
    %a = llvm.call @ascend_tile_load_f32(%arg0, %m, %d) : (!llvm.ptr<1>, i32, i32) -> i32
...
module {
  llvm.func @scatter_test(%arg0: !llvm.ptr<1>, %arg1: !llvm.ptr<1>, %arg2: !llvm.ptr<1>) attributes {hacc.entry} {
    ...
    %m = llvm.mlir.constant(8 : i32) : i32
    %d = llvm.mlir.constant(16 : i32) : i32
    %a = llvm.call @ascend_tile_load_f32(%arg0, %n, %d) : (!llvm.ptr<1>, i32, i32) -> i32
...
module {
  llvm.func @fill_test(%arg0: !llvm.ptr<1>, %arg1: !llvm.ptr<1>) attributes {hacc.entry} {
    ...
    %res = llvm.call @ascend_tile_fill_f32(%z, %z, %r, %c) : (i32, i32, i32, i32) -> i32
    llvm.call @ascend_tile_store_f32(%arg1, %res, %r, %c) : (!llvm.ptr<1>, i32, i32, i32) -> ()
    llvm.return
  }
}
...
module {
  llvm.func @unknown_test(%arg0: !llvm.ptr<1>, %arg1: !llvm.ptr<1>) attributes {hacc.entry} {
    ...
    %c = llvm.mlir.constant(64 : i32) : i32
    %a = llvm.call @ascend_tile_load_f32(%arg0, %r, %c) : (!llvm.ptr<1>, i32, i32) -> i32
    %res = llvm.call @ascend_tile_frobnicate_f32(%a, %a, %r, %c) : (i32, i32, i32, i32) -> i32
    llvm.call @ascend_tile_store_f32(%arg1, %a, %r, %c) : (!llvm.ptr<1>, i32, i32, i32) -> ()
    llvm.return
  }
}
"#;
        let py = convert_mlir_to_tpu(mlir).unwrap();
        assert!(
            py.contains("# TODO: unhandled intrinsic: ascend_tile_frobnicate_f32"),
    ...
    fn test_tpu_argmax_arity_error() {
        let e = call_err("%0 = llvm.call @ascend_tile_argmax_f32(%a, %b) : (i32, i32) -> i32");
        assert!(e.contains("expected 4 args"), "got: {e}");
    }

    #[test]
    fn test_tpu_sample_top_p_arity_error() {
        let e = call_err("%0 = llvm.call @ascend_tile_sample_top_p_f32(%a, %b) : (i32, i32) -> i32");
        assert!(e.contains("expected 7 args"), "got: {e}");
    }

    #[test]
    fn test_tpu_draft_verify_arity_error() {
        let e = call_err("%0 = llvm.call @ascend_tile_draft_verify_f32(%a, %b) : (i32, i32) -> i32");
        assert!(e.contains("expected 5 args"), "got: {e}");
    }

    #[test]
    fn test_tpu_token_accept_arity_error() {
        let e = call_err("%0 = llvm.call @ascend_tile_token_accept_f32(%a, %b) : (i32, i32) -> i32");
        assert!(e.contains("expected 6 args"), "got: {e}");
    }

    #[test]
    fn test_tpu_attention_gqa_arity_error() {
        let e = call_err("%0 = llvm.call @ascend_tile_attention_gqa_f32(%a, %b) : (i32, i32) -> i32");
...
module {
  llvm.func @fillf(%arg0: !llvm.ptr<1>) attributes {hacc.entry} {
    ...
    %res = llvm.call @ascend_tile_fill_f32(%v, %v, %r, %c) : (f32, f32, i32, i32) -> i32
    llvm.return
  }
}
"#;
...
module {
  llvm.func @ptr_thread(%arg0: !llvm.ptr<1>, %arg1: !llvm.ptr<1>) attributes {hacc.entry} {
    ...
    %a = llvm.call @ascend_tile_load_f32(%p, %r, %c) : (!llvm.ptr<1>, i32, i32) -> i32
    llvm.call @ascend_tile_store_f32(%arg1, %a, %r, %c) : (!llvm.ptr<1>, i32, i32, i32) -> ()
    llvm.return
  }
}
"#;
    ...
    fn test_tpu_arity_errs() {
        ...
        let cases: &[(&str, &str, &str)] = &[
            ...
            ("ascend_tile_max_f32", "%c0, %c1", "(i32, i32) -> i32"),        // binop_fn needs 5
            ("ascend_tile_exp_f32", "%c0, %c1", "(i32, i32) -> i32"),        // unary needs 4
            ("ascend_tile_log_f32", "%c0, %c1", "(i32, i32) -> i32"),        // unary needs 4
            ("ascend_tile_rsqrt_f32", "%c0, %c1", "(i32, i32) -> i32"),      // unary needs 4
            ("ascend_tile_sigmoid_f32", "%c0, %c1", "(i32, i32) -> i32"),    // unary needs 4
            ...
            ("ascend_tile_cast_f32_f16", "%c0, %c1", "(i32, i32) -> i32"),   // cast needs 4
            ("ascend_tile_cast_f16_f32", "%c0, %c1", "(i32, i32) -> i32"),   // cast needs 4
            ("ascend_tile_cast_bf16_f32", "%c0, %c1", "(i32, i32) -> i32"),  // cast needs 4
