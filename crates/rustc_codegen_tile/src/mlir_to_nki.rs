...
//! |---|---|
//! | `__tile_load_f32(gm, rows, cols)` | `nl.load(ptr[nl.mgrid[0:R, 0:C]])` |
//! | `__tile_store_f32(gm, buf, rows, cols)` | `nl.store(ptr[...], tile)` |
//! | `__tile_add_f32(0, a, b, rows, cols)` | `nisa.tensor_tensor(a, b, op=np.add)` |
//! | `__tile_sub_f32(0, a, b, rows, cols)` | `nisa.tensor_tensor(a, b, op=np.subtract)` |
//! | `__tile_mul_f32(0, a, b, rows, cols)` | `nisa.tensor_tensor(a, b, op=np.multiply)` |
//! | `__tile_exp_f32(0, src, rows, cols)` | `nisa.activation(np.exp, src)` |
//! | `__tile_softmax_f32(0, src, rows, cols)` | 5-step decomposition (trowmax → sub → exp → trowsum → div) |
//! | `__tile_matmul_f32(0, a, b, m, k, n)` | `nisa.nc_matmul(a, b)` |
//! | `__tile_reduce_max_f32(0, src, rows, cols)` | `nisa.tensor_reduce(np.max, src, axis=(1,))` |
//! | `__tile_reduce_sum_f32(0, src, rows, cols)` | `nisa.tensor_reduce(np.add, src, axis=(1,))` |
//! | `__tile_scale_f32(0, src, scalar, rows, cols)` | `nisa.tensor_scalar(src, np.multiply, scalar)` |
//! | `__tile_transpose_f32(0, src, rows, cols)` | `nisa.nc_transpose(src)` |
//! | `__tile_rsqrt_f32(0, src, rows, cols)` | `1.0 / nl.sqrt(src)` |
//! | `__tile_log_f32(0, src, rows, cols)` | `nl.log(src)` |
//! | `__tile_sigmoid_f32(0, src, rows, cols)` | decomposed: 1/(1+exp(-x)) |
//! | `__tile_clamp_f32(0, src, min, max, rows, cols)` | `nl.minimum(nl.maximum(src, min), max)` |
//! | `__tile_cast_f32_f16(0, src, rows, cols)` | `nl.cast(src, dtype=nl.float16)` |
//! | `__tile_cast_f16_f32(0, src, rows, cols)` | `nl.cast(src, dtype=nl.float32)` |
//! | `__tile_slice_f32(0, src, r_off, c_off, sr, sc, dr, dc)` | `src[r_off:r_off+dr, c_off:c_off+dc]` |
//! | `__tile_concat_f32(0, a, b, rows, cols_a, cols_b)` | `nl.concatenate((a, b), axis=1)` |
//! | `__tile_scatter_f32(0, src, idx, n, m, d)` | loop-based scatter |
//! | `__tile_gather_f32(0, src, idx, n, m, d)` | loop-based gather |
//! | `__tile_topk_f32(0, src, idx_out, rows, cols, k)` | sort + slice top-k |
//! | `__tile_argmax_f32(0, src, rows, cols)` | `nl.argmax(src, axis=1, keepdims=True)` |
//! | `__tile_sample_top_p_f32(0, logits, temp, top_p, seed, rows, cols)` | softmax + greedy argmax approx |
//! | `__tile_draft_verify_f32(0, draft, target, rows, cols)` | row-max + clamp to [0,1] |
//! | `__tile_token_accept_f32(0, draft, target, probs, thresh, rows)` | mask-select draft or target |
//! | `__tile_silu_f32(0, src, rows, cols)` | `x * sigmoid(x)` via `nisa.activation` |
//! | `__tile_cast_bf16_f32(0, src, rows, cols)` | `nisa.cast(data=src, dtype=nl.float32)` (bf16→f32) |
//! | `__tile_matmul_transposed_f32(0, a, b, m, k, n)` | `nisa.nc_matmul(a, transpose(b))` |
//! | `__tile_attention_gqa_f32(0, q, k, v, heads, head_dim, seq_len, kv_heads)` | matmul+softmax+matmul with GQA head grouping |
//! | `__tile_rope_f32(0, src, pos, rows, cols)` | Rotary Position Embedding (pair-wise cos/sin rotation) |
//!
...
//!
//! ⚠ Neither bound is enforced anywhere in this file, and the worked example
//! above emits `nl.mgrid[0:1, 0:1024]` — F = 1024, twice the figure this line
//! states. So the line is wrong, or the limit is per-instruction rather than
//! per-tile (in the Neuron ISA the free-dimension cap is a property of the
//! consuming instruction, and `nisa.nc_matmul` caps its contraction dimension at
//! 128 partitions, which a 128x256 @ 256x512 matmul would exceed). Settling it
//! needs Trainium or `neuronx-cc`, neither of which is available here, so no
//! refusal has been added: every other emitter in this tree refuses when it
//! knows a hardware limit (`mlir_to_pto` on UB overflow, `mlir_to_tilelang` on a
//! non-multiple matmul), and guessing this one would refuse kernels that may be
//! perfectly legal.
//!
    ...
    for line in mlir_text.lines() {
        ...
        while let Some(pos) = rest.find("@__tile_") {
            ...
            if !KNOWN_INTRINSICS.iter().any(|k| name.starts_with(k)) {
                return Err(format!(
                    "mlir_to_nki has no arm for `{name}`. Emitting it would drop the \
...
impl NkiContext {
    ...
    /// An operand that must be a tile some lowered op produced.
    ///
    /// The old form fell back to the raw SSA name, which put `%y` straight into
    /// the emitted Python — a file that does not parse. That happens exactly
    /// when the intrinsic which should have produced the tile was not lowered,
    /// so the consumer is where the omission first becomes visible. Say so
    /// there instead of emitting something no one can run.
    fn require_tile(&self, ssa: &str) -> Result<String, String> {
        self.get_tile_var(ssa).map(str::to_string).ok_or_else(|| {
            format!(
                "mlir_to_nki: {ssa} is used as a tile but no lowered op produced it; \
                 the intrinsic that should have produced it was not emitted"
            )
        })
    }

    /// An operand that may be a tile or a scalar constant — a quantization
    /// scale, a rope position. Falling back to the SSA name broke these the same
    /// way: a constant scale emitted `nl.divide(t0, %sc)`.
    fn require_tile_or_scalar(&self, ssa: &str) -> Result<String, String> {
        let key = ssa.trim();
        if let Some(v) = self.get_tile_var(key) {
            return Ok(v.to_string());
        }
        if let Some(f) = self.float_const_map.get(key) {
            return Ok(f.clone());
        }
        ...
        Err(format!(
            "mlir_to_nki: {ssa} is neither a tile any lowered op produced nor a \
...
fn translate_call(
    ...
    match callee.as_str() {
        ...
        "__tile_store_f32" | "__tile_store_f16" => {
            ...
            // Falling back to the raw SSA name put `%y` straight into the
            // emitted Python, which does not parse. That happens whenever the
            // intrinsic that should have produced the tile was not lowered —
            // an unsupported op, or one whose arity did not match — so the
            // store is the first place the omission becomes visible. Refuse
            // there rather than emit a file no one can run.
            let tile_var = ctx.require_tile(tile_ssa)?;
        ...
        "__tile_mul_f32" | "__tile_mul_f16" => {
            ...
            if args.len() >= 5 {
                if let Some((ref silu_out, ref silu_src)) = ctx.last_silu.clone() {
                    ...
                    let a_var = ctx.require_tile(a_ssa)?;
                    let b_var = ctx.require_tile(b_ssa)?;
                    if &a_var == silu_out || &b_var == silu_out {
                        ...
                        extra_ops.push(format!("{} = nl.divide({}, {})", t_sig, t_ones, t_denom));
        ...
        "__tile_exp_f32" | "__tile_exp_f16" => {
            ...
            let src_var = ctx.require_tile(src_ssa)?;
            let var = ctx.fresh_tile();
            if let Some(r) = &result {
                ctx.tile_vars.insert(r.clone(), var.clone());
                ctx.tile_shapes
        ...
        "__tile_softmax_f32" | "__tile_softmax_f16" => {
            ...
            Ok(Some(format!("{} = nl.divide({}, {})", var, t_exp, t_sum)))
        }
        ...
        "__tile_transpose_f32" | "__tile_transpose_f16" => {
            ...
            Ok(Some(format!("{} = nisa.nc_transpose({})", var, src_var)))
        }
        ...
        "__tile_topk_f32" | "__tile_topk_f16" => {
            ...
            extra_ops.push(format!("{} = {}[:, :{}]", t_topk_idx, t_sorted_idx, k));
            ...
            Ok(Some(format!("{} = {}[:, :{}]", var, t_sorted_vals, k)))
        }
        ...
        "__tile_rms_norm_f32" | "__tile_rms_norm_f16" => {
            ...
            // f16: square and reduce in f32. Narrowing at every step leaves the
            // sum-of-squares with only half-precision headroom; the reference
            // computes in f32 and the resulting rms disagrees by ~1.5 unit
            // roundoffs of half — above the run path's f16 budget. The mul by
            // src still narrows on assignment, so the stored tile stays f16.
            if dtype == "f16" {
                Ok(Some(format!(
                    "{v} = {s} * nl.rsqrt(nl.add(nisa.tensor_reduce(np.add, nl.multiply(nl.cast({s}, dtype=nl.float32), nl.cast({s}, dtype=nl.float32)), axis=[1], dtype=np.float32) / {c}, {e}))",
                    v = var,
                    s = src_var,
                    c = cols,
                    e = eps_val
                ...
                Ok(Some(format!(
                    "{v} = {s} * nl.rsqrt(nl.add(nisa.tensor_reduce(np.add, nl.multiply({s}, {s}), axis=[1], dtype={dt}) / {c}, {e}))",
                    v = var,
                    s = src_var,
                    c = cols,
                    e = eps_val,
        ...
        "__tile_quantize_f32_i8" => {
            ...
            let cols = ctx.resolve_const(args[4].trim());
            let src_var = ctx.require_tile(src_ssa)?;
            let scale_var = ctx.require_tile_or_scalar(scale_ssa)?;
            let var = ctx.fresh_tile();
            if let Some(r) = &result {
                ctx.tile_vars.insert(r.clone(), var.clone());
                ctx.tile_shapes
        ...
        "__tile_sample_top_p_f32" => {
            ...
            extra_ops.push(format!("{} = nl.divide({}, {})", t_probs, t_exp, t_sum));
        ...
        // ── __tile_token_accept_f32(dst, draft_tokens, target_tokens, accept_probs, threshold, rows) → u32 ──
        ...
        "__tile_token_accept_f32" => {
            ...
            let rows = ctx.resolve_const(args[5].trim());
            let draft_var = ctx.require_tile(draft_ssa)?;
            let target_var = ctx
                .get_tile_var(target_ssa)
                .unwrap_or(target_ssa)
                .to_string();
            let probs_var = ctx.require_tile(probs_ssa)?;
        ...
        "__tile_matmul_transposed_f32" | "__tile_matmul_transposed_f16" => {
            ...
            extra_ops.push(format!("{} = nisa.nc_transpose({})", t_bt, b_var));
        ...
        "__tile_attention_gqa_f32" | "__tile_attention_gqa_f16" => {
            ...
            // ⚠ DIVERGENT, AND KNOWN. tile_std now declares
            // __tile_attention_gqa_f32 as
            //     (dst, q, k, v, heads_q, heads_kv, seq_len, head_dim)
            // which is what mlir_to_cpp, mlir_to_gpu and mlir_to_aie already
            // implement. This arm reads args[5] as head_dim and args[7] as
            // kv_heads -- the two are swapped relative to the declaration, so
            // the same MLIR module lowered here and through mlir_to_cpp
            // computes different attention shapes.
            //
            // NKI support for this operator is POSTPONED by decision rather
            // than fixed here: correcting it changes Trainium device behaviour
            // and wants a device run to confirm, which this tree cannot do.
            // The divergence is recorded here rather than silently carried, so
            // whoever picks NKI up finds the contract and not a puzzle.
            //
            // To fix: swap to args[4]=heads_q, args[5]=heads_kv,
            // args[6]=seq_len, args[7]=head_dim, and update the fixture in
            // crates/codegen_tests/src/mlir_to_nki.rs which was written to
            // match this reading.
            ...
            let dtype = if callee.contains("f16") { "f16" } else { "f32" };
            let npdtype = np_dtype(dtype);
            let q_var = ctx.require_tile(q_ssa)?;
            let k_var = ctx.require_tile(k_ssa)?;
            let v_var = ctx.require_tile(v_ssa)?;

            // Transpose K for scores = Q @ K^T
            let t_kt = ctx.fresh_tmp();
            extra_ops.push(format!("{} = nisa.nc_transpose({})", t_kt, k_var));
            ...
            extra_ops.push(format!("{} = nl.divide({}, {})", t_attn, t_exp, t_sum));
        ...
        "__tile_rope_f32" | "__tile_rope_f16" => {
            ...
            let src_var = ctx.require_tile(src_ssa)?;
            let pos_var = ctx.require_tile_or_scalar(pos_ssa)?;
            let half = cols / 2;
...
mod tests {
    ...
    fn nki_for(name: &str, body: &str, ptrs: &[&str]) -> String {
        let args = ptrs
            .iter()
            .map(|p| format!("%{p}: !llvm.ptr<1>"))
            .collect::<Vec<_>>()
            .join(", ");
        let mlir = format!(
            "module {{\n  llvm.func @{name}({args}) attributes {{hacc.entry}} {{\n    ^bb0:\n{body}    llvm.return\n  }}\n}}\n"
        );
        convert_mlir_to_nki(&mlir).expect("emitter must accept the kernel")
    }

    /// Operands that may be a scalar rather than a tile — the quantization
    /// scale, rope's position — fell back to the raw SSA name when they were not
    /// tiles, so a constant scale emitted `nl.divide(t0, %sc)`: Python that does
    /// not parse. They resolve against the module's constants now.
    #[test]
    fn a_constant_scale_resolves_to_its_literal_not_its_ssa_name() {
        let body = "    %r = llvm.mlir.constant(8 : i32) : i32\n    %c = llvm.mlir.constant(64 : i32) : i32\n\
    %sc = llvm.mlir.constant(2.0e-2 : f32) : f32\n\
    %x = llvm.call @__tile_load_f32(%a, %r, %c) : (!llvm.ptr<1>, i32, i32) -> i32\n\
    %y = llvm.call @__tile_dequantize_i8_f32(%y, %x, %sc, %r, %c) : (i32, i32, f32, i32, i32) -> i32\n\
    llvm.call @__tile_store_f32(%b, %y, %r, %c) : (!llvm.ptr<1>, i32, i32, i32) -> ()\n";
        let py = nki_for("q", body, &["a", "b"]);
        assert!(py.contains("nl.multiply(t0, 2.0e-2)"), "scale must be its literal:\n{py}");
        assert!(!py.contains('%'), "no SSA name may survive into the Python:\n{py}");
    }

    /// A tile operand no lowered op produced is refused, not written out as its
    /// SSA name. `__tile_causal_mask_f32` has no NKI arm, so the store used to
    /// emit `nl.store(..., %y)`.
    #[test]
    fn an_unlowered_producer_is_refused_at_its_consumer() {
        let body = "    %r = llvm.mlir.constant(8 : i32) : i32\n    %c = llvm.mlir.constant(64 : i32) : i32\n\
    %x = llvm.call @__tile_load_f32(%a, %r, %c) : (!llvm.ptr<1>, i32, i32) -> i32\n\
    %y = llvm.call @__tile_causal_mask_f32(%y, %x, %r, %c) : (i32, i32, i32, i32) -> i32\n\
    llvm.call @__tile_store_f32(%b, %y, %r, %c) : (!llvm.ptr<1>, i32, i32, i32) -> ()\n";
        let args = "%a: !llvm.ptr<1>, %b: !llvm.ptr<1>";
        let mlir = format!(
            "module {{\n  llvm.func @m({args}) attributes {{hacc.entry}} {{\n    ^bb0:\n{body}    llvm.return\n  }}\n}}\n"
        );
        let err = convert_mlir_to_nki(&mlir).expect_err("must refuse, not emit %y");
        assert!(err.contains("no lowered op produced it"), "unhelpful refusal: {err}");
    }

    /// The same, one step earlier: an unlowered tile reaching an *operand*
    /// rather than the store. Thirty-nine operand sites shared the fallback, and
    /// a test that only drives the store cannot see any of them.
    #[test]
    fn an_unlowered_tile_reaching_an_operand_is_refused_too() {
        let body = "    %r = llvm.mlir.constant(8 : i32) : i32\n    %c = llvm.mlir.constant(64 : i32) : i32\n\
    %x = llvm.call @__tile_load_f32(%a, %r, %c) : (!llvm.ptr<1>, i32, i32) -> i32\n\
    %m = llvm.call @__tile_causal_mask_f32(%m, %x, %r, %c) : (i32, i32, i32, i32) -> i32\n\
    %y = llvm.call @__tile_add_f32(%y, %x, %m, %r, %c) : (i32, i32, i32, i32, i32) -> i32\n\
    llvm.call @__tile_store_f32(%b, %y, %r, %c) : (!llvm.ptr<1>, i32, i32, i32) -> ()\n";
        let mlir = format!(
            "module {{\n  llvm.func @o(%a: !llvm.ptr<1>, %b: !llvm.ptr<1>) attributes {{hacc.entry}} {{\n    ^bb0:\n{body}    llvm.return\n  }}\n}}\n"
        );
        let err = convert_mlir_to_nki(&mlir)
            .expect_err("an operand no lowered op produced must be refused");
        assert!(err.contains("%m"), "must name the operand at fault: {err}");
        assert!(err.contains("no lowered op produced it"), "unhelpful refusal: {err}");
    }

    /// Nothing asserted on the matmul emission: renaming `nisa.nc_matmul` across
    /// all nine of its sites left this suite green. The operand order is the
    /// part worth pinning — `nc_matmul` is not symmetric, and the transposed
    /// form has to put the transpose on the second operand, not the first.
    #[test]
    fn matmul_emits_nc_matmul_with_the_operands_in_order() {
        let body = "    %m = llvm.mlir.constant(64 : i32) : i32\n    %k = llvm.mlir.constant(128 : i32) : i32\n    %n = llvm.mlir.constant(256 : i32) : i32\n\
    %x = llvm.call @__tile_load_f32(%a, %m, %k) : (!llvm.ptr<1>, i32, i32) -> i32\n\
    %z = llvm.call @__tile_load_f32(%b, %k, %n) : (!llvm.ptr<1>, i32, i32) -> i32\n\
    %y = llvm.call @__tile_matmul_f32(%y, %x, %z, %m, %k, %n) : (i32, i32, i32, i32, i32, i32) -> i32\n\
    llvm.call @__tile_store_f32(%o, %y, %m, %n) : (!llvm.ptr<1>, i32, i32, i32) -> ()\n";
        let py = nki_for("mm", body, &["a", "b", "o"]);
        assert!(py.contains("t0 = nl.load(p0[nl.mgrid[0:64, 0:128]])"), "A tile:\n{py}");
        assert!(py.contains("t1 = nl.load(p1[nl.mgrid[0:128, 0:256]])"), "B tile:\n{py}");
        assert!(py.contains("nisa.nc_matmul(t0, t1)"), "operands in order:\n{py}");
        // The result is M x N, not M x K or K x N.
        assert!(py.contains("nl.store(p2[nl.mgrid[0:64, 0:256]]"), "result shape:\n{py}");
    }

    /// The transposed form must transpose the *second* operand and feed the
    /// result to nc_matmul, leaving the first alone.
    #[test]
    fn matmul_transposed_transposes_only_the_second_operand() {
        let body = "    %m = llvm.mlir.constant(64 : i32) : i32\n    %k = llvm.mlir.constant(128 : i32) : i32\n    %n = llvm.mlir.constant(256 : i32) : i32\n\
    %x = llvm.call @__tile_load_f32(%a, %m, %k) : (!llvm.ptr<1>, i32, i32) -> i32\n\
    %z = llvm.call @__tile_load_f32(%b, %k, %n) : (!llvm.ptr<1>, i32, i32) -> i32\n\
    %y = llvm.call @__tile_matmul_transposed_f32(%y, %x, %z, %m, %k, %n) : (i32, i32, i32, i32, i32, i32) -> i32\n\
    llvm.call @__tile_store_f32(%o, %y, %m, %n) : (!llvm.ptr<1>, i32, i32, i32) -> ()\n";
        let py = nki_for("mmT", body, &["a", "b", "o"]);
        let tr = py
            .lines()
            .find(|l| l.contains("nc_transpose"))
            .unwrap_or_else(|| panic!("no transpose emitted:\n{py}"))
            .to_string();
        assert!(tr.contains("nc_transpose(t1)"), "must transpose B, not A: {tr}");
        assert!(
            py.contains("nisa.nc_matmul(t0, _tmp0)"),
            "the transposed operand must reach nc_matmul in second position:\n{py}"
        );
    }
...
"#
    ...
    fn test_nki_gep_offset_mgrid() {
        ...
        assert!(
            py.contains("nl.mgrid[0:1, 0:1024]"),
            "missing zero-offset mgrid"
        );
        // Second load: offset 1024 / cols=1024 = row 1 → mgrid[1:2, 0:1024]
        assert!(
            py.contains("nl.mgrid[1:2, 0:1024]"),
            "missing offset mgrid for gep+1024"
        );
    }
...
module {
  llvm.func @transpose_test(%arg0: !llvm.ptr<1>, %arg1: !llvm.ptr<1>) attributes {hacc.entry} {
    ...
    llvm.call @__tile_store_f32(%arg1, %res, %c, %r) : (!llvm.ptr<1>, i32, i32, i32) -> ()
    llvm.return
  }
}
"#;
...
module {
  llvm.func @clamp_test(%arg0: !llvm.ptr<1>, %arg1: !llvm.ptr<1>) attributes {hacc.entry} {
    ...
    %a = llvm.call @__tile_load_f32(%arg0, %r, %c) : (!llvm.ptr<1>, i32, i32) -> i32
    %res = llvm.call @__tile_clamp_f32(%a, %a, %lo, %hi, %r, %c) : (i32, i32, i32, i32, i32, i32) -> i32
    llvm.call @__tile_store_f32(%arg1, %res, %r, %c) : (!llvm.ptr<1>, i32, i32, i32) -> ()
    llvm.return
  }
}
"#;
        let py = convert_mlir_to_nki(mlir).unwrap();
        assert!(
            py.contains("nl.maximum("),
            "missing nl.maximum for lower clamp"
        );
        assert!(
            py.contains("nl.minimum("),
            "missing nl.minimum for upper clamp"
        );
    }
...
"#;
        let py = convert_mlir_to_nki(mlir).unwrap();
        assert!(
            py.contains("nisa.tensor_tensor(") && py.contains("nl.maximum"),
            "max must use nl.maximum:\n{}",
            py
        );
    }
...
"#;
        let py = convert_mlir_to_nki(mlir).unwrap();
        assert!(
            py.contains("nisa.tensor_tensor(") && py.contains("nl.divide"),
            "div must use nl.divide:\n{}",
            py
        );
    }
...
"#;
        let py = convert_mlir_to_nki(mlir).unwrap();
        assert!(
            py.contains("nl.rsqrt("),
            "rms_norm must use nl.rsqrt:\n{}",
            py
        );
    }
...
"#;
        let py = convert_mlir_to_nki(mlir).unwrap();
        assert!(
            py.contains("nl.round("),
            "quantize must use nl.round:\n{}",
            py
        );
        assert!(
            py.contains("nl.clip("),
            "quantize must use nl.clip:\n{}",
            py
        );
    }
...
"#;
        let py = convert_mlir_to_nki(mlir).unwrap();
        assert!(
            py.contains("nl.multiply("),
            "dequantize must use nl.multiply:\n{}",
            py
        );
    }
...
"#;
        let py = convert_mlir_to_nki(mlir).unwrap();
        assert!(
            py.contains("nl.argmax("),
            "argmax must use nl.argmax:\n{}",
            py
        );
        assert!(
            py.contains("axis=1"),
            "argmax must reduce over axis=1:\n{}",
            py
        );
    }
...
"#;
        let py = convert_mlir_to_nki(mlir).unwrap();
        assert!(
            py.contains("nisa.activation(np.exp"),
            "sample_top_p must use exp:\n{}",
            py
        );
        assert!(
            py.contains("nl.argmax("),
            "sample_top_p must use argmax:\n{}",
            py
        );
    }
...
"#;
        ...
        assert!(
            py.contains("nisa.tensor_reduce(np.max"),
            "draft_verify must use row-max:\n{}",
            py
        );
        assert!(
            py.contains("nl.minimum("),
            "draft_verify must clamp to 1:\n{}",
            py
        );
    }
...
"#;
        let py = convert_mlir_to_nki(mlir).unwrap();
        assert!(
            py.contains("np.greater_equal"),
            "token_accept must use >= comparison:\n{}",
            py
        );
        assert!(
            py.contains("np.multiply"),
            "token_accept must use mask multiply:\n{}",
            py
        );
    }
...
"#;
        ...
        assert!(
            py.contains("np.multiply, -1.0"),
            "silu must negate for sigmoid:\n{}",
            py
        );
        assert!(
            py.contains("nisa.activation(np.exp"),
        ...
        assert!(
            py.contains("np.add, 1.0"),
            "silu must add 1 for denominator:\n{}",
            py
        );
        assert!(
            py.contains("nl.divide("),
            "silu must divide for sigmoid:\n{}",
            py
        );
        assert!(
            py.contains("op=np.multiply"),
            "silu must multiply x * sigmoid(x):\n{}",
            py
        );
    }
...
"#;
        let py = convert_mlir_to_nki(mlir).unwrap();
        assert!(
            py.contains("nisa.cast("),
            "cast_bf16_f32 must use nisa.cast:\n{}",
            py
        );
        assert!(
            py.contains("dtype=nl.float32"),
            "cast_bf16_f32 must target float32:\n{}",
            py
        );
    }
...
"#;
        let py = convert_mlir_to_nki(mlir).unwrap();
        assert!(
            py.contains("nisa.nc_transpose("),
            "matmul_transposed must transpose B:\n{}",
            py
        );
        assert!(
            py.contains("nisa.nc_matmul("),
            "matmul_transposed must use nc_matmul:\n{}",
            py
        );
    }
...
module {
  llvm.func @gqa_test(%arg0: !llvm.ptr<1>, %arg1: !llvm.ptr<1>, %arg2: !llvm.ptr<1>, %arg3: !llvm.ptr<1>) attributes {hacc.entry} {
    ...
    %q = llvm.call @__tile_load_f32(%arg0, %heads, %hd) : (!llvm.ptr<1>, i32, i32) -> i32
    %k = llvm.call @__tile_load_f32(%arg1, %seq, %hd) : (!llvm.ptr<1>, i32, i32) -> i32
    %v = llvm.call @__tile_load_f32(%arg2, %seq, %hd) : (!llvm.ptr<1>, i32, i32) -> i32
    %res = llvm.call @__tile_attention_gqa_f32(%q, %q, %k, %v, %heads, %hd, %seq, %kvh) : (i32, i32, i32, i32, i32, i32, i32, i32) -> i32
    llvm.call @__tile_store_f32(%arg3, %res, %heads, %hd) : (!llvm.ptr<1>, i32, i32, i32) -> ()
    llvm.return
  }
}
"#;
        ...
        assert!(
            py.contains("nisa.nc_transpose("),
            "gqa must transpose K:\n{}",
            py
        );
        assert!(
            py.contains("nisa.nc_matmul("),
            "gqa must use nc_matmul:\n{}",
            py
        );
        assert!(
            py.contains("nisa.tensor_reduce(np.max"),
            "gqa must have softmax rowmax:\n{}",
            py
        );
        assert!(
            py.contains("nisa.activation(np.exp"),
            "gqa must have softmax exp:\n{}",
            py
        );
        assert!(
            py.contains("nl.divide("),
            "gqa must have softmax divide:\n{}",
            py
        );
        // Check scale factor: 1/sqrt(64) = 0.125
        assert!(
            py.contains("0.125"),
            "gqa must scale by 1/sqrt(head_dim):\n{}",
            py
        );
    }
...
"#;
        ...
        assert!(
            py.contains("nisa.activation(np.exp"),
            "fused silu_mul must use exp:\n{}",
            py
        );
        // Must multiply silu result with up
        assert!(
            py.contains("op=np.multiply"),
            "fused silu_mul must use multiply:\n{}",
            py
        );
    }
...
"#;
        ...
        assert!(
            py.contains("op=np.multiply"),
            "standalone silu must have x * sigmoid(x):\n{}",
            py
        );
    }
...
"#;
        let py = convert_mlir_to_nki(mlir).unwrap();
        assert!(
            py.contains("nisa") || py.contains("sqrt") || py.contains("rms"),
...
"#;
        let py = convert_mlir_to_nki(mlir).unwrap();
        assert!(
            py.contains("multiply") || py.contains("*"),
            "missing scale in NKI output:\n{}",
            py
        );
        assert!(
            py.contains("add") || py.contains("+"),
...
"#;
        let py = convert_mlir_to_nki(mlir).unwrap();
        assert!(
            py.contains("matmul") || py.contains("dot"),
...
"#;
        ...
        assert!(
            py.contains("nl.arange"),
            "rope must build freq via arange:\n{}",
            py
        );
        assert!(
            py.contains("nisa.activation(np.exp"),
            "rope must use exp for freq:\n{}",
            py
        );
        // Must compute cos and sin of angle
        assert!(
            py.contains("nl.cos("),
            "rope must compute cos(angle):\n{}",
            py
        );
        assert!(
            py.contains("nl.sin("),
            "rope must compute sin(angle):\n{}",
            py
        );
        // Must split into even/odd and recombine
        assert!(
            py.contains("0::2") || py.contains("0:8:2"),
            "rope must slice even cols:\n{}",
            py
        );
        assert!(
            py.contains("1::2") || py.contains("1:8:2"),
            "rope must slice odd cols:\n{}",
            py
        );
        // Must have subtract for out_even = even*cos - odd*sin
        assert!(
            py.contains("op=np.subtract"),
            "rope must subtract for even output:\n{}",
            py
        );
        // Must interleave back via stack+reshape
        assert!(
            py.contains("nl.stack("),
            "rope must interleave via stack:\n{}",
            py
        );
        assert!(
            py.contains(".reshape("),
            "rope must reshape after stack:\n{}",
            py
        );
    }
...
"#;
    ...
    fn test_nki_arity_errs() {
        ...
        let cases: &[(&str, &str, &str)] = &[
            ...
            ("__tile_max_f32", "%c0, %c1", "(i32, i32) -> i32"),  // binop needs 5
            ("__tile_exp_f32", "%c0, %c1", "(i32, i32) -> i32"),  // needs 4
            ...
            ("__tile_rsqrt_f32", "%c0, %c1", "(i32, i32) -> i32"), // needs 4
            ("__tile_log_f32", "%c0, %c1", "(i32, i32) -> i32"),  // needs 4
            ("__tile_sigmoid_f32", "%c0, %c1", "(i32, i32) -> i32"), // needs 4
            (
                "__tile_clamp_f32",
                "%c0, %c1, %c2",
                "(i32, i32, i32) -> i32",
            ), // needs 6
            ("__tile_cast_f32_f16", "%c0, %c1", "(i32, i32) -> i32"), // needs 4
            ("__tile_cast_f16_f32", "%c0, %c1", "(i32, i32) -> i32"), // needs 4
            ("__tile_cast_bf16_f32", "%c0, %c1", "(i32, i32) -> i32"), // needs 4
            (
                "__tile_slice_f32",
                "%c0, %c1, %c2",
                "(i32, i32, i32) -> i32",
