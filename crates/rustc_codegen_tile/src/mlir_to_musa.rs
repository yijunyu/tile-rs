...
"#
    ...
    fn test_musa_header() {
        ...
        assert!(
            !mu.contains("cuda_runtime.h"),
            "should not contain cuda_runtime.h"
        );
    ...
    fn test_musa_kernel_syntax_unchanged() {
        ...
        // The softmax warp reduction moved to the butterfly form, which gives
        // every lane the result rather than only lane 0. Both intrinsics still
        // exist in the emitter; this kernel uses the xor one.
        assert!(mu.contains("__shfl_xor_sync"), "missing __shfl_xor_sync");
...
"#
    ...
    fn test_musa_silu_mul_fusion() {
        ...
        // Must be MUSA-ified
        assert!(
            mu.contains("musa_runtime.h"),
            "must have MUSA header:\n{}",
            mu
        );
        assert!(
            !mu.contains("cuda_runtime.h"),
            "must NOT have CUDA header:\n{}",
            mu
        );
    }
}
