...
// 15 of 16 share the signature `(mlir: &str) -> Result<String, String>`; only
...
fn register_emitters(r: &mut TargetRegistry) {
    ...
    r.register(Box::new(EmitterTarget::new("rvv", "mlir", convert_mlir_to_rvv)));
}

/// Adapter that turns a plain `(mlir: &str) -> Result<String, String>` emitter
/// (the 15 uniform backends) into a [`CodegenTarget`]. Std-only and ALWAYS
